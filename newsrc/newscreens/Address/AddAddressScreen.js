...
const AddAddressScreen = ({  route }) => {
  ...
  const handleSave = async () => {
    ...
    const dbData = mapUIToDatabase(formData, userId, addressId);
    
    try {
      console.log('Saving to DB:', dbData);
      
      // Prepare new address data to send back to SelectLocationScreen
      const newAddressData = {
        id: Date.now(), // Temporary ID for new address
        label: formData.label,
        address_line1: formData.flatNumber,
        address_line2: [
          formData.buildingBlock,
          formData.landmark,
          formData.areaName
        ].filter(Boolean).join(', '),
        city: formData.city,
      ...
      Alert.alert(
        'Success',
        addressId ? 'Address updated successfully' : 'Address saved successfully',
        [{
          text: 'OK',
          onPress: () => {
            if (!addressId) {
              // For new addresses, pass the data back to SelectLocationScreen
              navigation.navigate('SelectLocationScreen', { newAddress: newAddressData });
            } else {
              // For edited addresses, just go back
